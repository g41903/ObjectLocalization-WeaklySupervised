...
class WSDDN(nn.Module):
    ...
    def __init__(self, classes=None, debug=False, training=True):
        ...
        self.criterion = nn.BCELoss(size_average=False).cuda()
...
#         x = torch.Size([1, 256, 36, 55])
        
        x = self.roi_pool(x, network.np_to_variable(rois, is_cuda=True))
#         x = torch.Size([2997, 256, 6, 6])
        
        x = x.view(x.size(0), 256 * 6 * 6)
#         x= torch.Size([2997, 9216])

        x = self.classifier(x)
#         x = torch.Size([2997, 4096])
        
        
        cls_score = self.score_cls(x)
#         cls_score = torch.Size([2997, 20])

        det_score = self.score_det(x)
#         det_score = torch.Size([2997, 20])
...
#         det_score = torch.Size([2997, 20])
        ...
        cls_prob = torch.mul(cls_score, det_score)
#         cls_prob = torch.Size([2997, 20])       
    ...
    def build_loss(self, cls_prob, label_vec):
        ...
        # cls_prob = torch.Size([128, 20]); label_vec = torch.Size([2, 20])
        criterion = nn.BCEWithLogitsLoss().cuda()
        loss = 0
        label_vec = label_vec.view(1,20)
        cls_prob = torch.sum(cls_prob,dim=0).view(1,20)
        cls_prob = torch.clamp(cls_prob, max = 0.9999)
