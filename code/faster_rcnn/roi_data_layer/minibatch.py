...
def get_weak_minibatch(roidb, num_classes):
    ...
    for idx in xrange(num_images):
        labels, overlaps, im_rois, bbox_targets, bbox_inside_weights \
            = _sample_rois(roidb[idx], fg_rois_per_image, rois_per_image,
                           num_classes)
        ...
        labels_idx = np.unique(roidb[idx]['gt_classes']) # find which classes appeared in the roidb: ex: roidb[idx]['gt_classes'] = [0,3,3,3,0] np.unique() = [0,3]
        for l_i in labels_idx:
            if l_i>0:
                labels[0, l_i-1] = 1
        
        # Add to RoIs blob, im_rois = (2997, 4), rois_per_image= : rois = (:, 4)
        # find top 64 rois in the images
        rois = _project_im_rois(im_rois, im_scales[idx])[:,:]
        batch_ind = idx * np.ones((rois.shape[0], 1)) # batch_ind = (:,1)
        rois_blob_this_image = np.hstack((batch_ind, rois)) # rois_blob_this_image = (64,5)
