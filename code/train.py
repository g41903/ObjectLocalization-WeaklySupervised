...
if use_visdom:
    ...
    loss_win = viz.line(X=np.array([0]),Y=np.array([0]),opts=dict(markers=False))
    ap_win = viz.line(X=np.array([0]),Y=np.array([0]),opts=dict(markers=False))
...
#             viz.updateTrace(X=step, Y=loss.data[0], win=loss_win,name='train_loss')
    ...
    if is_plot_loss and step%500==0:
        #TODO: Create required visualizations
#         import pdb; pdb.set_trace()
        if use_tensorboard:
    ...
    if is_plot_weights and step%1000==0:
        #TODO: Create required visualizations
        if use_tensorboard:
