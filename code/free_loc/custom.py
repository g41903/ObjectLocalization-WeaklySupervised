...
class LocalizerAlexNet(nn.Module):
    ...
    def init_weights(m,o):
#         import pdb; pdb.set_trace()
        if type(o) == nn.Conv2d:
            nn.init.xavier_normal(o.weight.data)


    def forward(self, x):
        #TODO: Define forward pass
        # x = torch.Size([32, 3, 512, 512])
        x = self.features(x)
        # x = torch.Size([32, 256, 31, 31])
        x = self.classifier(x) 
        # x = torch.Size([32, 20, 29, 29])
        return x
...
#     import pdb; pdb.set_trace()
    if pretrained:
        ...
        for item_name in own_weights.keys():
            print("load pretrained module: {}".format(item_name))
